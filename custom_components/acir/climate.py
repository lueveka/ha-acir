...
from homeassistant.components.climate.const import (
    ...
    SWING_ON, SWING_OFF,
    SUPPORT_SWING_MODE, HVAC_MODES, ATTR_HVAC_MODE, FAN_MEDIUM)
...
def setup_platform(
    ...
    """Set up platform."""
    _LOGGER.warn(f"debug1")
    add_entities([IRClimate()])

class IRClimate(ClimateEntity):
    def __init__(self) -> None:
        _LOGGER.warn(f"debug2")
...
#         _LOGGER.debug(f"IRClimate init started for device {config.get(CONF_NAME)}")
#         self.hass = hass
...
#         self._manufacturer = 'todo manufacturer'
#         self._supported_models = 'todo supportedModels'
#         self._supported_controller = 'Broadlink'
#         self._commands_encoding = 'Base64'
#         self._min_temperature = 16
#         self._max_temperature = 31
#         self._precision = 1

#         valid_hvac_modes = [HVAC_MODE_AUTO]

#         self._operation_modes = [HVAC_MODE_OFF] + valid_hvac_modes
#         self._fan_modes = [FAN_MEDIUM]
...
#         _LOGGER.warn("send_command not implemented")
